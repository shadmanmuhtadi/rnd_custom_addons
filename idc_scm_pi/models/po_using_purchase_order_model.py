...
class PurhcasePo(models.Model):
    ...
    order_line = fields.One2many('purchase.order.line.copy', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True)

    pi_lines = fields.One2many('purchase.order.pi.copy','po_id', string='', copy=False)
...
#----------------------------------------------------------------------------    
    def action_open_pi(self):
        ...
        return {
            ...
            'view_id': False,
            'view_type': 'form',
            'res_model': 'purchase.order.pi.copy',
    ...
    def create(self, vals):
        ...
        if vals.get('name', 'New') == 'New':
            ...
            vals['name'] = self_comp.env['ir.sequence'].next_by_code('purchase.order.copy', sequence_date=seq_date) or '/'
...
class PurhcasePoline(models.Model):
    _name = "purchase.order.line.copy"
    _inherit = ['purchase.order.line']
    _description = "Purchase order line copy"


    order_id = fields.Many2one('purchase.order.copy', string='Order Reference', index=True, required=True, ondelete='cascade')
