...
class PurhcasePi(models.Model):
    ...
    order_line = fields.One2many('purchase.order.pi.line.copy', 'order_id', string='Order Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True)
    po_id = fields.Many2one('purchase.order.copy', string='Purchase Order')
...
#--------------------------------------------------------------------------
    ...
    def create(self, vals):
        ...
        if vals.get('name', 'New') == 'New':
            ...
            vals['name'] = self_comp.env['ir.sequence'].next_by_code('purchase.order.pi.copy', sequence_date=seq_date) or '/'
...
class PurhcasePiline(models.Model):
    _name = "purchase.order.pi.line.copy"
    _inherit = ['purchase.order.line']
    _description = "Purchase order pi line copy"


    order_id = fields.Many2one('purchase.order.pi.copy', string='Order Reference', index=True, required=True, ondelete='cascade')
